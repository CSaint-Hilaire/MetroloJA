...
def create_XYZ_box(df_XYZ, param, table_column_param, med_column_param, im_path,
                   result, ttest_table_column, delta1, delta2, XY_delta, df_MedStd_SBR, leg_dict, sys_name, 
    ...
    for i in range(len(date1)-1):
        ...
        fig.add_annotation(dict(font=dict(size=15), 
                                x=X, y=float(Yx)+delta1+f_delta2,
                                text=symbolX,
                                showarrow=False,
                                arrowhead=1,
                                xref='x1',
        ...
        fig.add_annotation(dict(font=dict(size=15), 
                                x=X, y=float(Yy)+delta1+f_delta2,
                                text=symbolY,
                                showarrow=False,
                                arrowhead=1,
                                xref='x1',
...
def select_param():
    ...
    master.title('What measure do you want to plot ?')
    ...
    for key, val in values.items():  # for name, age in dictionary.iteritems():  (for Python 2.x)
        if val == which_button_is_selected:
            selected_param = [key]


    if which_button_is_selected == "5":
        master2 = tk.Tk()
    ...
    return(which_button_is_selected, selected_param, values)


    
    

def display_selected_plot(values, which_button_is_selected, selected_param, df_XYZ, df_SBR, dfXYZ_MedStd, df_MedStd_SBR, folder_selected, leg_dict):
    main = tk.Tk()
    main.withdraw()
    ...
    elif which_button_is_selected == "3":
        param = 'Mes./theory resolution ratio'
    ...
    if int(which_button_is_selected) in range(1, 4):
        create_XYZ_box(df_XYZ, param, table_column_param, med_column_param, im_path,
                       result, ttest_table_column, delta1, delta2, XY_delta, df_MedStd_SBR, leg_dict, sys_name, 
                       dfX_MedStd, dfY_MedStd, dfZ_MedStd)

    elif int(which_button_is_selected) == 4:
